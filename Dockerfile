...
# Define build argument for optional tidymodels installation
ARG INSTALL_TIDYMODELS=true
...
RUN if [ "$INSTALL_TIDYMODELS" = "true" ]; then \
    Rscript -e "if (!requireNamespace('recipes', quietly = TRUE)) remotes::install_version('recipes', version='1.0.6', repos='https://cloud.r-project.org', Ncpus=4)"; \
    Rscript -e "if (!requireNamespace('rsample', quietly = TRUE)) remotes::install_version('rsample', version='1.2.1', repos='https://cloud.r-project.org', Ncpus=4)"; \
    Rscript -e "if (!requireNamespace('tune', quietly = TRUE)) remotes::install_version('tune', version='1.1.1', repos='https://cloud.r-project.org', Ncpus=4)"; \
    Rscript -e "if (!requireNamespace('broom', quietly = TRUE)) remotes::install_version('broom', version='1.0.5', repos='https://cloud.r-project.org', Ncpus=4)"; \
